...
//    private int dividerColor = 0xffcccccc;
    //tab选中颜色
    private int textSelectedColor = 0xffFFA500;
    //tab未选中颜色
    ...
    public MyDropDownMenu(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, com.zxl.library.R.styleable.DropDownMenu);
        textSelectedColor = a.getColor(com.zxl.library.R.styleable.DropDownMenu_ddtextSelectedColor, textSelectedColor);
        needSetSelectedColor = a.getBoolean(com.zxl.library.R.styleable.DropDownMenu_ddneedSetSlectedColor, needSetSelectedColor);
        textUnselectedColor = a.getColor(com.zxl.library.R.styleable.DropDownMenu_ddtextUnselectedColor, textUnselectedColor);
        menuBackgroundColor = a.getColor(com.zxl.library.R.styleable.DropDownMenu_ddmenuBackgroundColor, menuBackgroundColor);
        maskColor = a.getColor(com.zxl.library.R.styleable.DropDownMenu_ddmaskColor, maskColor);
        menuTextSize = a.getDimensionPixelSize(com.zxl.library.R.styleable.DropDownMenu_ddmenuTextSize, menuTextSize);
        menuMaxHeight = a.getDimensionPixelSize(com.zxl.library.R.styleable.DropDownMenu_ddmenuMaxHeight, menuMaxHeight);
        menuSelectedIcon = a.getResourceId(com.zxl.library.R.styleable.DropDownMenu_ddmenuSelectedIcon, menuSelectedIcon);
        menuUnselectedIcon = a.getResourceId(com.zxl.library.R.styleable.DropDownMenu_ddmenuUnselectedIcon, menuUnselectedIcon);
        a.recycle();
    ...
    public GridView cryView;
    public ConstellationAdapter cryAdapter;
    public String[] cryArr;
        ...
        for (int i = 0; i < viewDatas.size(); i++) {
            ...
            switch (key) {
                ...
                case TYPE_GRID:
                    if (value instanceof String[] && select_position < ((String[]) value).length) {
                        cryArr = (String[]) value;
                        view = setGridView(i, select_position);
                    }
                    else
    ...
    private View setGridView( final int index, int select_position) {
        cryAdapter = new ConstellationAdapter(getContext(), Arrays.asList(cryArr));
        LayoutInflater li = LayoutInflater.from(getContext());
        View v = li.inflate(R.layout.drop_menu_grid_layout2, null);
        v .setBackground(ContextCompat.getDrawable(tabMenuView.getContext(), R.drawable.menu_bg));
        cryView = (GridView) v.findViewById(R.id.constellation);
        if (select_position != -1) {
            cryAdapter.setCheckItem(select_position);
            setTabText(index, cryArr[select_position]);
        }
        cryView.setAdapter(cryAdapter);
        cryView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
    ...
    private void addTab(@NonNull List<String> tabTexts, int i) {
        ...
        tab.setPadding(dpTpPx(5), dpTpPx(6), dpTpPx(5), dpTpPx(6));
        tab.setBackground(ContextCompat.getDrawable(tabMenuView.getContext(),R.drawable.menu_bg));;
        //添加点击事件
